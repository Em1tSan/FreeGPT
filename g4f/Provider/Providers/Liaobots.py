import os, uuid, requests, random # Import random module
...
def _create_completion(model: str, messages: list, stream: bool, **kwargs):
  ...
  while True:
      ...
      with open('liaobotskeys.txt', 'r') as f:
          keys = f.readlines()
      # Choose a random line from the list and strip the newline character
      auth_key = random.choice(keys).strip()

      json_data = {
          'conversationId': str(uuid.uuid4()),
          'model': models[model],
          'messages': messages,
          # Use the random line as the value of auth
          'auth': auth_key,
          'key': '',
